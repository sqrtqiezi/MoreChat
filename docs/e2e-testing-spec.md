...
E2E 测试中的 Mock 范围必须严格收敛。以下约束为强制要求：

1. **只 Mock 聚合 Bot 接口**: 允许 Mock 的范围仅限聚合 Bot 的上游接口或聚合 Bot 对外暴露的集成边界，用于隔离第三方依赖、不稳定外部系统或难以稳定构造的机器人返回。
2. **禁止 Mock 当前项目 API**: MoreChat 当前系统自身提供的前端、后端、数据库、会话、消息、目录、搜索以及所有 `/api/*` 接口，在 E2E 中必须使用当前测试环境的真实实现，禁止在浏览器端、Step、Hook 或 support helper 中伪造其返回。
3. **禁止新增前端 API Mock**: 后续新增或改造 E2E 场景时，不得再引入针对当前项目 API 的 `page.route()`、拦截器假响应或等价替代方案。
4. **存量 Mock 必须下线**: 已存在的浏览器侧系统内 API Mock 仅允许作为迁移中的临时遗留，必须在对应场景迁移完成后立即删除，且相关文档说明也必须同步从项目中移除。
5. **优先保证集成真实性**: E2E 的目标是验证真实系统协作，而不是验证浏览器侧伪造返回；应优先通过环境准备、测试数据准备和系统配置解决问题，而不是扩张 Mock 面。
