...
- **当前实现**：使用进程内内存队列（`p-queue`），不引入 Redis/BullMQ 等外部依赖
- **当前限制**：队列任务不会持久化；进程重启后，内存中的待处理任务会丢失
- **当前控制方式**：通过单进程并发控制限制同时处理的任务数，避免资源争抢
- **后续可选增强**：如果后面发现摘要、主题聚类或其他异步任务对“重启可恢复”有刚性需求，再评估将待处理任务 ID 或运行状态落到 SQLite
